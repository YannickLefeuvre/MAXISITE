...
public class VilleResource {
    ...
    public ResponseEntity<Ville> updateVille(@PathVariable(value = "id", required = false) final Long id, @Valid @RequestBody Ville ville)
    ...
    public ResponseEntity<Ville> partialUpdateVille(
        @PathVariable(value = "id", required = false) final Long id,
        @NotNull @RequestBody Ville ville
    ) throws URISyntaxException {
        ...
        Optional<Ville> result = villeRepository
            ...
            .map(existingVille -> {
                ...
                if (ville.getIsCapital() != null) {
                    existingVille.setIsCapital(ville.getIsCapital());
                }
                if (ville.getIcone() != null) {
                    existingVille.setIcone(ville.getIcone());
                }
                if (ville.getIconeContentType() != null) {
                    existingVille.setIconeContentType(ville.getIconeContentType());
                }
                if (ville.getAbsisce() != null) {
                    existingVille.setAbsisce(ville.getAbsisce());
                }
                if (ville.getOrdonnee() != null) {
                    existingVille.setOrdonnee(ville.getOrdonnee());
                }
                if (ville.getArriereplan() != null) {
                    existingVille.setArriereplan(ville.getArriereplan());
                }
                if (ville.getArriereplanContentType() != null) {
                    existingVille.setArriereplanContentType(ville.getArriereplanContentType());
                }
     ...
     */
    @GetMapping("/villes")
    public List<Ville> getAllVilles(@RequestParam(required = false) String filter) {
        if ("lienorigine-is-null".equals(filter)) {
            log.debug("REST request to get all Villes where lienOrigine is null");
            return StreamSupport
                .stream(villeRepository.findAll().spliterator(), false)
                .filter(ville -> ville.getLienOrigine() == null)
                .collect(Collectors.toList());
        }

        if ("liencible-is-null".equals(filter)) {
            log.debug("REST request to get all Villes where lienCible is null");
            return StreamSupport
                .stream(villeRepository.findAll().spliterator(), false)
                .filter(ville -> ville.getLienCible() == null)
                .collect(Collectors.toList());
        }
