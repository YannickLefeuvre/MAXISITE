...
/**
 * REST controller for managing {@link com.mycompany.myapp.domain.Serie}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class SerieResource {
    ...
    public ResponseEntity<Serie> partialUpdateSerie(
        ...
        Optional<Serie> result = serieRepository
            ...
            .map(existingSerie -> {
                if (serie.getNom() != null) {
                    existingSerie.setNom(serie.getNom());
                }
                if (serie.getImages() != null) {
                    existingSerie.setImages(serie.getImages());
                }
                if (serie.getImagesContentType() != null) {
                    existingSerie.setImagesContentType(serie.getImagesContentType());
                }
